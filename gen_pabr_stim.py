...
n_minutes = 2  # number of minutes' worth of epochs to make
...
if np.any(f_band):  # if doing tonebursts
    ...
    This means that the tonebursts are the same size, and the clicks are half
    the size. It makes things a little bit cleaner.
...
for rate in [20, 40, 80, 120, 160, 200]:
    rand = np.random.RandomState(np.array(f_band).astype(int))
